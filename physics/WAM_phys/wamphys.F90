...
!  con_pi, con_rerth, con_p0,  con_g, con_omega,  
!  con_cp, con_rd, con_rv, con_fvirt
!==========================
...
! local variables
!==========================================================
    integer, parameter  :: kdtin = 2400    !for 5 day spin up!!!!   ! JAN1 year start DEc26 2022 for 2023 run  
!    integer, parameter  :: kdtin = 14400    !for 30 day spin up!!!!   ! apr1year 2024 


!!!!    integer, parameter :: kdtin = 3360      ! 7 days spin start 25 dec 2023 for 2024 year run 
    ...
    real(kind=kind_phys), dimension(im, levs) :: rdelp, del       ! rdelp(i,k) = 1./(PRSI(i,k) - PRSI(i,k+1))
    real(kind=kind_phys), dimension(im, levs) :: kappa            ! mid-lev values k=R/cp for eddy heat cond
    real(kind=kind_phys), dimension(im, levs) :: amu2d,   jo2_out
    real(kind=kind_phys), dimension(im, levs) :: ekin
...
!	print *, ' kdt-kdtin-kdts ', kdt, kdtin, kdts
    if (me == master ) then
    
      if ((kdts .le. 0)  .and. (kdts .gt. kdt_swmax) ) then
       print *, ' kdt-kdtin-kdts ', kdt, kdtin, kdts
       stop ' kdts out of range '
      endif
    endif
...
!      iday_m = jdat(3)
!      imo = jdat(2)
!print *, '3 kdt-kdtin-kdts ', kdt, kdtin, kdts	
    call wamphys_kpf107_update(kdts,  jdat(1), jdat(2), jdat(3), ddd_year, solhr, &
                   sol_f107, sol_f107a, sol_kp, sol_kpa, me, master)  
!print *, '3 kdt-kdtin-kdts ', kdt, kdtin, kdts		    		    
    f107  = sol_f107
    f107d = sol_f107a
    kp    = sol_kp
    ...
    if (me == master ) then
     if (kdts < 100) print *, 'spw-drivers f-kp ', f107, f107d, kp, kpa
!     print *, 'spw-drivers f-kp ', f107, f107d, kp, kpa
 
     if (kp .gt. 8) then 
          print *, ' kpf107 >8  ', f107, kp,  kdt
          print *,  ' bt, bz, sdn, svl', sol_bt, sol_bz, sol_den, sol_vel 
          print *,  ' nhp, shp, npi, spi', sol_nhp, sol_shp, sol_nhpi, sol_shpi	  	  	   
     endif
    endif	     
    do k=1, levs 
...
!       
    ...
    call wamphys_tracer_run(me, master,im, levs,ntrac,ntrac_i,nto1, nto2, nto3,   &
                            ...
                            o_n,o2_n, o3_n, n2_n,nair,rho,am, amu2d, cospass,     &
                            zgeo, jo2_out, f107,f107d, go2dr, plow, phigh, xpk_low, xpk_high)
...
! for day take instant values radiation every timestep
    ...
    call wam_ion_run(im, levs, jdat, prsl, solhr,cospass,zgeo, grav, o_n, o2_n,n2_n,&
	        ...
	        sol_nhp, sol_nhpi, sol_shp, sol_shpi, sol_bt, sol_ang, sol_vel, sol_bz, sol_den)	    
