...
!==========
!2025-04 V. A. Yudin => empirical paramters  of GSL "plolevmeso,    facmeso" do not work for FV3WAM
! 
   logical, parameter :: oro_fv3wam =  .true.
!=========      
...
! vay-2025   
   real(kind=kind_phys)                 :: rcpdt   !  1./cp/deltim 
...
!          FV3WAM-HR 
!      
!vay-2025  cdmbgwd(1:2)      = [20.0 - Block, 2.5 -OGW],   1.0, 1.0    
      if (oro_fv3wam) then  
         kpblmax = km/4              ! 196 /4 = 49 levels of Fv3wam   
!	 cleff = 0.5E-5 / SQRT(FLOAT(IMX)/192.0)  
         cleff = 6.28e-3/600.        ! for c96 Lh = 6*dx =600 km
      endif	 
!      
...
! vay-2025 
             if (.not.oro_fv3wam) ldrag(i) = ldrag(i) .or. sqrt(bnv2(i,1))*rulow(i).lt.ksmax
...
!
         ...
         enddo
!
! vay  nonsense of lcap = km
!
     if  (.not.oro_fv3wam) then
         if(lcap.lt.km) then
            do klcap = lcapp1,km
...
! rstoch(i) =0.	    
	  if (.not.oro_fv3wam)   then
            ...
            taud_ms(i,k)  = taud_ms(i,k)*dtfac(i)*dtfac_meso*ls_taper(i)
	                      			       
            taud_bl(i,k)  = taud_bl(i,k)*dtfac(i)* ls_taper(i) 
...
! vay-2025 FV3WAM => heating "mandatory" due to orographic GWs	  
  
	      eng0 = 0.5*( u1(i,k)**2. + v1(i,k)**2. )
              eng1 = 0.5*( ((u1(i,k)+(dtaux+dtauxb)*deltim))**2 +  &
                            ((v1(i,k)+(dtauy+dtauyb)*deltim))**2 )	
!			    
! Qheat = (u*dudt+v*dvdx)*cp/deltim > by definition for stationary waves
!			    
	      dtdt(i,k) = dtdt(i,k)  +	rcpdt* &
                       max(dudt(i,k)*u1(i,k) + dvdt(i,k)*v1(i,k), 0.0)
!	      dtdt(i,k) = dtdt(i,k) + max((eng0-eng1),0.0) *rcpdt
