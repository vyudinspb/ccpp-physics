!***********************************************************************
!***********************************************************************
! File h2ocup.f created for data storage, initialization, and 
! calculation of H2O IR cooling rates in the rotational and 6.3-mum
! vibrational bands after Xun Zhu (1999) reusing some of his code and 
! data
! September, 2007: made by Rashid Akmaev for a pressure grid going 
!       upward
! 
! g1rtxz => g1rtxz_w
! g1vbxz_w
...
! Subroutine to initialize calculations of H2O IR cooling rates done 
! by h2occ after Zhu (1994)
! - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
! Nov 12, 2008: corrected errors in calculation of gdp
! Sep 24, 2007: made from h2ocin for upward model grid
! - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
      use machine,                only: kind_phys
      use wam_h2ocm
      implicit none
...
! Arguments
! INPUT
! -mid-layer model pressure (Pa) grid levels going up
      integer,intent(in):: lx, me, master
      real(kind=kind_phys),intent(in),dimension(lx):: p0

! -directory where input files are located
!hmhj character(len=*),intent(in):: dir

! - OUTPUT: placed in the module
! - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
! Internal parameters
      integer,parameter:: lmr=3,lmt=1
      
      real(kind=kind_phys),parameter:: delr0=.5,refpre=1./3.e3
...
! Work space
      ...
      real(kind=kind_phys),dimension(lx,lmr,lmt):: gamyrt,gamyvb
      real(kind=kind_phys),dimension(lx) ::  gh2ort,gh2ovb,dg1rt,dg2rt    
      real(kind=kind_phys),dimension(lx) ::  dg1vb,dg2vb,gdp,xx,wvmmrc,coeff     
! - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
...
! internal arrays go down for convenient calculation in the 
! cooling-to-space approximation. lh2oc is the starting model level
! (counting down from top) corresponding to approximately 100 km. Above
! that level the cooling rates are extrapolated to 0 within one scale
! height (within ltop1 model layers). If the model top does not reach
! 100 km, lh2oc=1, ltop1=0; if the model bottom is above 100 km, 
! lh2oc > lx and cooling rates are set to 0 in the model domain.

      

! Precalculate parameters for matrix interpolation

! Prepare reference atmosphere on model grid and other grid params. In
! the call to wvrefm_w, the model grid is inversed (goes down from the 
...
! - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
! Sep 24, 2007: Made from h2oc_calc for upward pressure grid
! October 1, 2003
! - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
      use machine,                only: kind_phys
      use wam_h2ocm
...
! - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
! Arguments
! - IN: temperature (K), pressure (Pa), and H2O MMR (relative units)
!       on the same model grid going up as in h2ocin
      integer:: lx
      real(kind=kind_phys),intent(in),dimension(lx):: t,p0,wvmmr
      
      real(kind=kind_phys),dimension(lx) ::  gh2ort,gh2ovb,dg1rt,dg2rt    
      real(kind=kind_phys),dimension(lx) ::  dg1vb,dg2vb,gdp,xx,wvmmrc,coeff 
      
! - OUT: heating rates (K/s) in the rotational and vibrational bands, 
! respectively
      real(kind=kind_phys),dimension(lx),intent(out):: qr,qv
! - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
! Internal parameters
! - inverse of reference H2O MMR
      real(kind=kind_phys),parameter:: rrmmr=1./(3.e-6)

! Work space (most of it kept for historic compatibility reasons)
      integer:: l,lu,lw
      real(kind=kind_phys):: phiv,thr,thv,gr2,gv2,wk1,wk2,yy
! - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
      qr(1:lx)=0.
      qv(1:lx)=0.
...
! Calculate cooling rates up to layer lh2oc. Index l goes down, lu goes
...
!
! 2*(4.3E-20/20.5)*air density in m^-3 (Coefficient 2 is due to
!     rearranged expression for calculation of qv)
!
...
!	 print *, lu, L, qv(lu), qr(lu), 'qv-qu-extrap', ltop1,   coeff(L)  
         enddo
      endif
...
!***********************************************************************
!***********************************************************************
! End of file h2ocup.f
!***********************************************************************
!***********************************************************************

!***********************************************************************
! File h2olib contains 10 subroutines/s used by h2oc.f
! Nov 13, 2008: updated
!     subroutine wvrefm_w
! Sept, 2007: Assembled by Rashid Akmaev
!***********************************************************************
...
! Internal parameters and work space
      ...
      real(kind=kind_phys),dimension(km) :: GG,WW
      real(kind=kind_phys) :: XK2(KFS,NQ,KM,MM4)
      real(kind=kind_phys) :: PRE(KFS),TEM4(MM4),T77(MM4)
      real(kind=kind_phys) :: XKUS(KUS,NQ,KM,MM4)
      integer info
...
!        size(gg),mpi_real4
 ...
 10   CONTINUE
  ...
  33    CONTINUE
        ...
        CALL interk_w(kus,kfs,km,mm4,nq,pre,xk2,pus,xkus)

        DO 80 N=1,NQUS
          VNQS(N)=V00+DDV*(FLOAT(N)-0.5)
          CALL qsingl_w(WK1,WK2,RUS,PUS,TUS,WK3,WK4,KUS,   &
...
!
          DO 90 N=1,NQUS
            ...
            FACX1=STRB(K,N)*blac_w(VNQS(N),TUS(K))
...
! Internal parameters and work space
      ...
      real(kind=kind_phys) ::  blac_w
      real(kind=kind_phys) :: FACX1
      integer :: M, N, N1, K, K2, K1, ICON, KFSM1, LLR, LLT
! - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
...
! Sept, 2007: made by Rashid Akmaev from Xun Zhu's code for H2O cooling
      ...
      real(kind=kind_phys),intent(in):: gamrt(kus,lmr),gamvb(kus,lmr)

      real(kind=kind_phys),intent(out)::  c1rt(kus),c2rt(kus),c1vb(kus),c2vb(kus)
...
! - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
!
!    To calculate the cooling rate by a single line located at VM in 1/m
!    with width DVM. PRE = pressure in pascal, TEM = temperature in Kelvin, 
!    SI = k-coefficient at GI in m*m/kg, RX = mass mixing ratio
!    Q1 & Q2 = cool-to-space & heat exchange cooling rate in K/day,
!    ITOP=0 ==> PRE(KM)->0;   ITOP=1 ==> PRE(1)->0.
!
...
! - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
      use machine,                only: kind_phys
      use  wamphys_math_interp, only  : splin1
      implicit none
      integer,intent(in):: lmy
      integer,intent(out):: lh2o,llin
      real(kind=kind_phys),dimension(lmy),intent(in):: pmy
      real(kind=kind_phys),dimension(lmy),intent(out):: coeff,wvmy,tmy
! - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
      integer:: l
      real(kind=kind_phys),dimension(lmy):: xmy
! - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
! Log-pressure above which cooling rates are not calculated but 
! extrapolated to 0 over a scale height (15 roughly corresponds to 
! 100 km)
      real(kind=kind_phys),parameter:: xtop=15.

! Empirical model of H2O MMR and T at 0-115 km based on Xun's models
      integer,parameter:: kz0=116
      real(kind=kind_phys),parameter,dimension(kz0):: x0=(/           &
      ...
      real(kind=kind_phys),parameter,dimension(kz0):: mmr0=(/         &
      ...
      real(kind=kind_phys),parameter,dimension(kz0):: tem0=(/         &
...
! - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
! Note, in the call from uh2oci the model grid is inversed (going down)
      do l=1,lmy
         xmy(l)=alog(1.e5/pmy(l))
      enddo
!     print*,'www1',pmy

! Determine the first model layer from the top below or about 100 km
      lh2o=lmy+1
      do l=1,lmy
         if(xmy(l) <= xtop) then
            lh2o=l
...
! For linear extrapolation of cooling rates upward of xmy(lh2o), count
! how many model layers are between xmy(lh2o) and xmu(lh2o)+1 and 
...
!***********************************************************************
      ...
      real(kind=kind_phys),intent(in):: t,t77(mm4),xkus(kus,nq,km,mm4)

      integer:: jj,jj1,jj3,jja,jjb
      real(kind=kind_phys):: y,dy,xt,WR1(3),WR2(3)
...
! - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
! File history
! July 2009: Modified from
!       hh2oh.f
! for discrete differencing, upward pressure grid, making heating
! rate -> 0 between 7 and 12 scale heights (as recommended by Victor in
! July 2007). The heating rate is now calculated in W/kg.
! Apr 06 2012  Henry Juang, initial implement for nems
! May    2022  Svetlana Karol ccpp-versio of h2ohdc.f => wamphys_h2oc.F90
! - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
      use machine,                only: kind_phys
      implicit none
...
! Formal arguments
!     IN:
! ctheta - cosine of solar zenith angle
! p0 - model layer pressure (Pa) grid going up
! mmr - H2O MMR (relative units) on model pressure grid going up
...
! Log-pressure of the uppermost extent of the parameterization around
! xtop = 12 sh as recommended by Victor (July 2007), the corresponding
! number of starting model layer, and the fade-off factor between
! xbot = 7 and xtop.
! ***xbot and xtop are assumed to be inside model domain!***
      real(kind=kind_phys),parameter:: xbot=7.,xtop=12.,rdx=1./(xtop-xbot)
      integer:: last
      real(kind=kind_phys):: factor(lx)

! Specific H2O data
! h2omu - molecular mass (g/mol)
      real(kind=kind_phys),parameter:: h2omu=18.015

! Parameterization data after C.D. Walshaw:
! - inverse reference pressure (1/Pa)
      real(kind=kind_phys),parameter:: rpref=1./101325.
! - number of bands
...
! "Rodgers factor" to crudely account for sphericity
         rodfac=35./sqrt(1224.*ctheta**2+1.)

! Scan pressure grid (now every call in case the grid changes),
...
! Begin with vertical-column H2O mass calculation for the entire column
         work(:)=mmr(:)/grav(:)

! Mass above the uppermost model layer assuming hydrostatics (diffusive
! equilibrium
...
! Above l=last the heating is zero
         work(last+1:lx)=0.

! For nonzero-heating layers calculate some more preliminaries
! In the top and bottom layers use one-sided differences
         work(last)=ctheta*grav(last)*factor(last)/(p0(last-1)-p0(last))
         work(1)=ctheta*grav(1)*factor(1)/(p0(1)-p0(2))

! In other layers use centered differences
         do l=2,last-1
...
! Calculate heating rate (W/kg) where nonzero
         do i=1,iband

! In the top and bottom layers use one-sided differences
            h2ohr(last)=h2ohr(last)+work(last)*(wb(last-1,i)-wb(last,i))
