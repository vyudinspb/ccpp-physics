...
!
! Calls wam_wam_spline interpolation routines from file wam_splin.f
!     wam_splin1
!     wam_splin2
!
...
! - Start diffusive equilibrium at
      ...
      real(kind=kind_phys), parameter:: xdiff=16.5
...
! Fomichev et al. (1998). Pressure calculated in co2cin
      ...
      real(kind=kind_phys),dimension(ivic0)::      co2vic,pvic
      real(kind=kind_phys),parameter,dimension(ivic0):: xvic0=(/      &          
...
! Victor's initial profile
      real(kind=kind_phys),parameter,dimension(ivic0):: co2vi0=(/ &                       
...
!
      real(kind=kind_phys),parameter:: xinter=16.5
...
!
      real(kind=kind_phys),dimension(43,57)::      vamat,vbmat
!
...
!
      real(kind=kind_phys)        co2nir(imnir)

! - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
! Parameterization data
! Log-pressure grid for heating calculation
!
      real(kind=kind_phys),parameter,dimension(imnir):: xnir=(/  &                      
...
!
      real(kind=kind_phys),parameter:: xtopn=15.5
      real(kind=kind_phys),parameter:: rdxnir=1./(xtopn-xnir(imnir))
!
! Tables of log(CO2 column amounts) and normalized heating rates
! taken and rearranged from Victor's pco2nir
!
      real(kind=kind_phys),parameter,dimension(10,imnir):: lnco2n=reshape((/  &          
      ...
      real(kind=kind_phys),parameter,dimension(10,imnir):: qco2n=reshape((/  &           
...
! - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
      ...
      use wam_co2pro_mod
      use wamphys_math_interp, only : wam_splin1
      implicit none
...
!
      integer:: l,lradif
      real(kind=kind_phys):: vicmu(ivic0)
      real(kind=kind_phys):: co2my(lmodel)
! - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
! Check parameter consistency
!
      if(xvic0(ivic0)<12.5) then
         write(6,*) '***Stop in co2pro_pre: Full Victor grid too low?'
...
!
      call wam_splin1(xmodel,mumod,xvic0,vicmu,lmodel,ivic0)
...
!
      call wam_splin1(xvic0,co2vic,xmodel,co2my,ivic0,lradif-1)
!
! Above xdiff use diffusive equilibrium if necessary (if lradif>lmodel,
!       the loop is not executed)
!
      do l=lradif,lmodel
         co2my(l)=exp(alog(co2my(l-1))+(xmodel(l)-xmodel(l-1))*  &
...
!
      ...
      real(kind=kind_phys),parameter,dimension(icrist)::     &                           
          xcrist=(/ 9.0, 10.0, 13., 15.5, 17.0 /),           &           
          cvcris=(/ 360.e-6, 355.e-6, 260.e-6, 100.e-6, 40.e-6 /)
...
!
      ...
      enddo
!
! Do wam_spline interpolation for these levels
!
      call wam_splin1(xcrist,cvcris,xvic0(lvic1:lvic2),   &                  
...
! - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
      use machine,                only: kind_phys
      use wam_co2c_mod
      use wam_co2pro_mod, only:xvic0,pvic,co2vic
      use wamphys_math_interp, only : wam_splin2
! - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
      implicit none
...
!       lhr < ltemp)
!
      integer,intent(in):: im,ltemp,lhr
!
...
!
      real(kind=kind_phys),intent(in):: temp(im,ltemp),xtemp(ltemp),   &                 
          xhr(lhr),mu(im,lhr),ro1(im,lhr),ro2(im,lhr),rn2(im,lhr)
      real(kind=kind_phys),intent(in):: co2my(ltemp)	  
!
! OUTPUT
! - output heating rate (J/kg)
!
      real(kind=kind_phys),intent(out):: hrate(im,lhr)
...
! RAA: increase collisional deactivation rate
       real(kind=kind_phys),parameter:: zco2o=3.e-12
...
!
      real(kind=kind_phys),dimension(im,itm50):: vicn2,vico2,vico1,vicmu
!
! Temp workspace
!
      integer:: i,iwork,j,l
      real(kind=kind_phys):: d1,d2,work1,work2,zn2,zo2,ztotal
      real(kind=kind_phys):: vict(im,ivict),vich(im,ivict-8),vics(im,ivict),   &         
          vlamb(im,itm50),htilda(im),flux(im)

! - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
! Boltzmann constant multiplied by 1e6 to convert pressure in Pa to
!     number density in 1/cm**3
!
      real(kind=kind_phys),parameter:: bol106=1.3807e-17

! - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
! Victor's data (with his original comments):
! ak = h*c/k*V - to determine the Planck function, k - Boltzmann's 
!     constant
      real(kind=kind_phys),parameter:: ak=960.217
!
! a10   - Einstine's (***Victor's classic spelling!!!***) coefficient 
!     for the fundamental band for the main isotope (1/s)
      real(kind=kind_phys),parameter:: eina10=1.5988
!
...
!
      real(kind=kind_phys),parameter:: conmy=2.55521e7

! - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
! Calculate composition on Victor grid above x=12.5 (index 50) for 
!       recurrent formula
!
      call wam_splin2(xhr,mu,xvic0(51),vicmu,lhr,itm50,im,im)
      call wam_splin2(xhr,rn2,xvic0(51),vicn2,lhr,itm50,im,im)
      call wam_splin2(xhr,ro2,xvic0(51),vico2,lhr,itm50,im,im)
!
! Feb 28, 2008
! idea change: the following portion of the code commented out and 
!     replaced by RAA
! For O interpolation find first model layer with [O]>.0
!
!      iwork=0
!      do j=1,im
!         do l=1,lhr
...
!c        print*,'www',iwork,ro1(1:im,iwork-1)
!         write(6,*) iwork,xhr(iwork)
!         stop
!      endif
!      call wam_splin2(xhr(iwork),ro1(1:im,iwork:),xvic0(51),vico1,         
!     &     lhr-iwork+1,itm50,im,im)
!
      call wam_splin2(xhr,ro1,xvic0(51),vico1,lhr,itm50,im, im)
!
...
! using wam_spline for T interpolation
!
      call wam_splin2(xtemp,temp,xvic0,vict,ltemp,ivict,im,im)
      do i=1,ivict
         do j=1,im
...
!
      do i=2,itm50
         ...
         do j=1,im
            ...
            vich(j,i+42)=conmy*co2vic(i+50)*htilda(j)*(1.-vlamb(j,i))/vicmu(j,i) 
         enddo
      enddo
!
! Transfer heating back from parameterization grid at x=2-xinter to
! model heating-rate grid by wam_spline interpolation.
!
      call wam_splin2(xvic0(9:),vich,xhr,hrate,iwork,lraint-1,im,im)
...
! - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
      ...
      use wam_co2c_mod
      use wam_co2pro_mod, only:   xvic0,pvic,co2vic
      use wam_qnir_mod,   only:   imnir,co2nir
...
!
      real(kind=kind_phys),parameter:: ana=6.022e23
!
! Temp work space
!
      integer:: i,j,l,info
      real(kind=kind_phys):: w1,wcolmy(lmod),war1(100),war2(100)
      real(kind=kind_phys):: vmu(ivict),vgrav(ivict)
...
!
      real(kind=kind_phys) :: uref(4), co2int(4), uco2(ivict)
...
!
      if(xmod(lmod).lt.xinter) then
         ...
         write(6,*) '***ERROR in wam_co2cin: model UB below Xinterface***'
         write(6,*) ' lmod= ',lmod 
         write(6,*) ' xmod(lmod)= ',xmod(lmod)
         write(6,*) ' xinter= ',xinter
        if (me == master) then
        do l=1,lmod
	   print *, 'sik-co2 ', xmod(l), pmod(l), mu(l), gr(l)
	enddo	 
	endif 
!         stop
      endif
...
!
! F98 model profile
      call wam_co2pro_pre(xmod,lmod,mu, co2my)
...
!       interpolate
      ...
      enddo
      j=i-1
      call wam_splin1(xmod,mu,xvic0(i),vmu(i),lmod,ivict-j)
      call wam_splin1(xmod,gr,xvic0(i),vgrav(i),lmod,ivict-j)
!
...
!
      if(xinter.ge.14.) then
         ...
         write(6,*) 'Error in wam_co2cin: xinter < 14'
!         stop
      endif
...
!
      do i = 1, 43
         do j = 1, 57
            ...
            co2int(3)=a540(i,j)
            co2int(4)=a720(i,j)
            call wam_splin1(uref,co2int,war1,war2,4,1)
!
...
! This part adopted from parcof with some modifications: wam_spline
...
! parameterization coeficients are equal escape function.
      ...
      do i=1,6
         if(uco2(i+50).lt.uco2ro(1)) then
            ...
            call wam_splin1(uco2ro,alo,uco2(i+50),war1,51,1)
         endif
         co2int(1)=cor150(i)
         co2int(2)=cor360(i)
         co2int(3)=cor540(i)
         ...
         call wam_splin1(uref,co2int,uco2(i+50),war2,4,1)
...
!
      ...
      end subroutine wam_co2cin

!***********************************************************************

      subroutine wam_qnirc(ct,x,co2mr,qnirh,lx)
...
! - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
      use machine,                only: kind_phys
      use wam_qnir_mod
      use wamphys_math_interp, only : wam_lint1
...
!       the heating rate
!
      real(kind=kind_phys),intent(in):: ct,x(lx),co2mr(lx)
!
! OUTPUT
!-heating rate in K/s (does not have to be divided by Cp)
!
      real(kind=kind_phys),intent(out):: qnirh(lx)
...
!
      ...
      integer,save:: lmnir
      logical:: first=.true.
      real(kind=kind_phys):: g,dx,hr(imnir)
...
!
         do l=1,imnir
            ...
            call wam_lint1(lnco2n(1,l),qco2n(1,l),10,qco2n(1,l),   &         
                0.,g,hr(l))
         enddo
!
...
!
         do l=1,lmnir
            call wam_lint1(xnir,hr,imnir,xnir(1),xnir(imnir),x(l),   &       
                qnirh(l))
